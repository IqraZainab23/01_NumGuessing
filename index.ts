...
class NumberGuessingGame {
    ...
    while (true) {
      if (userLife !== 0) {
        ...
        const userInput = await inquirer.prompt([
          {
            name: "userNumber",
            type: "number",
            message: chalk.italic.magentaBright.bold(
        ...
        if (
          ...
          if (differece === 0) {
            console.log(
              chalk.bgMagenta.bold(
                ` Amazing! You guessed it right! The number was correct.${targetNumber} `
              )
            );
            break;
          } else if (differece < 2) {
            console.log(
              chalk.blue.bold.italic(
                "Oops, very close! Only one digit away from stunning me!\n"
              )
            );
            userLife--;
          } else if (differece <= 2) {
          ...
          console.log(
            chalk.red.bold(`Kindly provide a number that falls within the designated range.`)
          );
        }
      } else {
        console.log(
          chalk.bgRed(` Oh no!Game Over... The number was ${targetNumber} `)
        );
        break;
      }
    }
  ...
  async restartGame() {
    ...
    if (restartGame.userDecision === endingChoices[0]) {
      ...
      const endingAnimation = chalkAnimation.pulse(
